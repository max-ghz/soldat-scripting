...
begin
	...
	ShowCubeInfo(255);
	writeconsole(0, '[Sandbox] Scripts are recompiling, wait a few seconds...', $f7e076);
end;
...
begin
	...
	if (NumPlayers-Spectators < 3) then begin
		if (not auto) then writeconsole(0, 'Too few players. Balance is disabled.', $FF0000) else writeln('<cube>  Too few players. Balance is disabled.');
		exit;
	end;
	...
	case lowercase(text) of
		'!balance', '!bal', '!teams', 'balance', 'bal', 'teams', 'balance?', 'bal?', 'teams?', '/balance', '/bal', '/teams': begin
			...
			if (GetPlayerStat(id, 'Team') = 5) then begin
				writeconsole(id, 'Spectators can''t use this trigger.', $FF0000);
			...
			if (bal.lastbalance < 15) then begin
				writeconsole(0, 'Last balance was less than 15 seconds ago. Please be patient.', ColorRed);
				exit;
			end;
...
begin
	...
	if (lowercase(getpiece(text, ' ', 0)) = '/gamemode') and (length(text) >= 11) then begin
		...
		writeconsole(0, '[Sandbox] Changing gamemode, please rejoin.', $f7e076);
...
begin
	if (lowercase(text) = '/bal') then if (GetPlayerStat(id, 'Team') <> 5) then if ((GameStyle = 3) or (GameStyle = 5)) then if (bal.lastbalance > 0) then begin
		...
		if (bal.lastbalance < 15) then begin
			writeconsole(id, 'Last balance was less than 15 seconds ago. Please be patient.', ColorRed);
			exit;
		end;
